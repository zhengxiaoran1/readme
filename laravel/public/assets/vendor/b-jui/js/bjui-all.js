            ...
            var initWebUploader = function($element, index) {
                ...
                if (options) {
                    ...
                    if (typeof options === 'object') {
                        ...
                        // 上传成功
                        uploader.on('uploadSuccess', function(file, responsed) {
                            console.log(responsed)
                            if (responsed[BJUI.keys.statusCode] != BJUI.statusCode.ok) {
                                BJUI.alertmsg('error', responsed.message)
                            } else {
                                var $li = $wrap.find('#'+ file.id +'_'+ index)
                                console.log(responsed.extend);
                                $li.find('input.upload').remove().end()
                                    .append('<input type="hidden" class="upload" name="'+ (options.upname || $element.data('name')) +'" value="'+ responsed.extend.photoId +'">')
                            }
                        })
                        // 上传失败
